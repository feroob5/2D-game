...
int stage1(SDL_Surface *fenetre, int *stag)
{ 
 
 Uint32 t_prev,dt=1;
 Mix_Music *song,*music;
 SDL_Surface *start=NULL,*fond=NULL, *quit=NULL,  *rotation = NULL, *X=NULL;
 ...
 SDL_Rect posdattex,poswin,posstart,posX;
 ...
 initialiser_personnageP(&perso);
 ie= init_ennemi(&e,"ennemi_spr.png");
 // layem kilberrima.
...
case SDL_MOUSEBUTTONDOWN:
{
	...
	if(event.button.button==SDL_BUTTON_RIGHT)
	{
		...
		if ((event.button.x>=0)&&(event.button.y>=0))
		{
			...
			animation_personnage_clavier(&perso,sens);
			camera=scrolling(sens,camera,&perso,&b);
			update_ennemi(&e, perso.position);

			// verification des conditions...
			if((collision_BoundingBox(&perso,&e)==0) &&(verifennemi==0)) // collision avec ennemi 1...
			{
				heal.nb-=50;
				score.nb-=5;
				verifennemi=1;
...
if(Collision_trigonometrique_inscr_iscr(perso.position,o14.position)== 1 &&  (o14.detection == 0)) //collision trigo 
	...
	if(event.key.keysym.sym == SDLK_SPACE)// ynaguez
	{
		sens=3;
		   deplacement_personnage(&perso,sens,&b);
	       perso.position.y-=50;
...
SDL_BlitSurface(X, NULL, b.fenetre, &posX);// X du score..
